...
impl RoleKind {
    ...
    pub fn match_user(&self, user: &UserInfo) -> bool {
        match self {
            ...
            RoleKind::Member => user.staker == true,
...
pub fn default_policy(council: Vec<AccountId>) -> Policy {
    Policy {
        ...
        proposal_period: U64::from(1_000_000_000 * 60 * 5 ),
    }
}
...
impl Policy {
    ...
    pub fn proposal_status(
        ...
        for role in roles {
            ...
            let threshold = ((role_info.kind.get_role_size().unwrap() as u128) / 2 ) as u128 + 1;
            ...
            env::log_str(vote_counts[Vote::Approve as usize].to_string().as_str());
            env::log_str(vote_counts[Vote::Reject as usize].to_string().as_str());
            env::log_str(vote_counts[Vote::Remove as usize].to_string().as_str());
            env::log_str(threshold.to_string().as_str());
